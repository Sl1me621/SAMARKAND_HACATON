...
class TrainedRAGWithExactCopy:
        ...
        for domain_name, domain_data in self.categories['domains'].items():
            ...
            for subcategory in domain_data.get('subcategories', []):
                ...
                for filename in subcategory.get('files', []):
                    ...
                    if file_path.exists():
                        templates.update(self.parse_template_file_exact(file_path, domain_name, subcategory['name']))
        
        return templates
    
    def parse_template_file_exact(self, file_path, domain, subcategory):
        """Парсит файлы с ТОЧНЫМ сохранением ответов"""
        templates = {}
            ...
            for section in sections:
                if section.strip():
                    ...
                    if len(lines) >= 2:
                        ...
                        # СОХРАНЯЕМ ТОЧНЫЙ ОРИГИНАЛЬНЫЙ ОТВЕТ БЕЗ ИЗМЕНЕНИЙ
                        exact_answer = '\n'.join(lines[1:]).strip()
                        
                        if question_pattern and exact_answer:
                            # Создаем несколько вариантов вопроса для лучшего поиска
                            search_variants = self.generate_search_variants(question_pattern)
                            
                            for variant in search_variants:
                                templates[variant] = {
                                    'exact_answer': exact_answer,  # ТОЧНЫЙ ответ из файла
                                    'domain': domain,
                                    'subcategory': subcategory,
                                    'source': f"{domain}/{subcategory}/{file_path.name}",
                                    'original_question': question_pattern
                                }
            
            print(f"📋 Загружено {len(templates)} точных ответов из {file_path.name}")
    ...
    def prepare_few_shot_examples(self):
        ...
        for question, data in list(self.qa_templates.items())[:10]:
            examples.append({
                'question': question,
                'answer': data['exact_answer']  # Используем точные ответы
            })
    ...
    def log_interaction(self, question, answer, used_template=False, confidence=0.0, user_feedback=None, answer_source="generated"):
        ...
        log_entry = {
            ...
            'answer_source': answer_source,  # 'exact_copy' или 'generated'
            'response_time': None
        }
    ...
    def get_statistics(self):
        ...
        exact_copy_usage = sum(1 for log in self.interaction_logs if log.get('answer_source') == 'exact_copy')
        avg_confidence = sum(log['confidence'] for log in self.interaction_logs) / total
        
        return {
            'total_interactions': total,
            'template_usage_rate': template_usage / total,
            'exact_copy_rate': exact_copy_usage / total,
    ...
    def prepare_retraining_data(self):
        ...
        for question, data in self.qa_templates.items():
            training_data.append({
                'input': question,
                'output': data['exact_answer'],  # Используем точные ответы
...
6. СТРОГО ЗАПРЕЩЕНО ОБРАЩЕНИЕ К ИНСТРУКЦИИ И УПОМИНАНИЕ ЕЕ И ШАГОВ, КОТОРЫЕ ЗАДАНЫ В НЕЙ
...
"""
    ...
    def analyze_usage_patterns(self):
        ...
        analysis = {
            ...
            'exact_copy_rate': (df['answer_source'] == 'exact_copy').mean(),
    ...
    def find_exact_match(self, question):
        ...
        # 2. Поиск по ключевым словам с ВЫСОКИМ порогом
        best_match = None
        best_score = 0
        ...
        for template_key, template_data in self.qa_templates.items():
            ...
            if score > best_score:
                best_score = score
                best_match = template_data['exact_answer']  # ТОЧНЫЙ ответ
                best_template_key = template_key
        
        # ВЫСОКИЙ ПОРОГ для точного копирования
        if best_match and best_score >= 0.8:  # 80% совпадение
    ...
    def calculate_exact_match_score(self, user_question, template_question):
        ...
        user_word_list = user_question.split()
        template_word_list = template_question.split()
        
        order_bonus = 0
        for i, word in enumerate(template_word_list[:4]):  # Первые 4 слова
            if i < len(user_word_list) and user_word_list[i] == word:
                order_bonus += 0.1
        
        return min(score + order_bonus, 1.0)
...
if __name__ == "__main__":
    ...
    def interactive_mode():
        ...
        print("🤖 RAG СИСТЕМА С ТОЧНЫМ КОПИРОВАНИЕМ ГОТОВА К РАБОТЕ")
        print("Команды:")
